...
def rules(file_name):
	# open needed files
	f = codecs.open('.\\test_data\\test_text.txt', 'r', 'utf-8')
	text = f.read()
	f.close()
	...
	for sent in return_sent(text):
		...
		for word in words:
			...
			# check pronouns
			check_in(pronouns, 'pron')

			# check adpositions
			check_in(adpositions, 'adl')
	...
	fs = []
	for ps in ['adj', 'conj', 'adv', 'part', 'v', 'n', 'pron', 'num', 'adl']:
		i = 0
		tp = 0
		fp = 0
		tn = 0
		fn = 0
		t = codecs.open('.\\test_rules.csv', 'r', 'utf-8')
		for line in t:
			line = line.strip()
			line_sp = line.split(';')
			if ', ' in line_sp[-1]:
				tags_test = line_sp[-1].split(', ')
				if ps in out[i] and ps in tags_test:
					tp += 1
				if ps in out[i] and ps not in tags_test:
					fp += 1
				if ps not in out[i] and ps in tags_test:
					fn += 1
				else:
					tn += 1
			else:
				if ps in out[i] and ps == line_sp[-1]:
					tp += 1
				if ps in out[i] and ps != line_sp[-1]:
					fp += 1
				if ps not in out[i] and ps == line_sp[-1]:
					fn += 1
				else:
					tn += 1
			i += 1
